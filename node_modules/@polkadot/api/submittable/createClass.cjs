...
function createClass(_ref2) {
  ...
  class Submittable extends ExtrinsicBase {
    ...
    signAndSend(account, partialOptions, optionalStatusCb) {
      ...
      return decorateMethod(() => this.#observeSign(account, options).pipe((0, _rxjs.switchMap)(getUpdateId => {
        return isSubscription ? this.#observeSubscribe(getUpdateId()) : this.#observeSend(getUpdateId());
    ...
    #observeSend = (() => {
      ...
      return function () {
        ...
        return api.rpc.author.submitExtrinsic(_this).pipe((0, _rxjs.tap)(hash => {
          _this.#updateSigner(updateId, hash);
        }));
      };
    })();
    #observeSubscribe = (() => {
      ...
      return function () {
        let updateId = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : -1;
        const txHash = _this2.hash;
        return api.rpc.author.submitAndWatchExtrinsic(_this2).pipe((0, _rxjs.switchMap)(status => _this2.#observeStatus(txHash, status)), (0, _rxjs.tap)(status => {
          _this2.#updateSigner(updateId, status);
        }));
      };
    })();
